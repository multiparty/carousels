...
var createMetric = function(spec) {
  ...
  return function () {
    var zero = math.parse('0'),  // create constant polynomial always = 0
        ...
        plus = function () { return new math.OperatorNode('+', 'add', Array.from(arguments)); },
        binplus = function (a, b) { return plus(a, b); },
      ...
      visitor: {
          ...
          "exit": function (p) {
            ...
            if (op_name == "map" || op_name == "reduce") {
              ...
              var length_poly = (
                                ...
                                ;
              p.node.metric = plus(dot(length_poly, func.metric), arg.metric);
            } else if (op_name in spec) {
              var arguments = p.node.arguments;
              arguments = arguments.map(e => e.metric);
              p.node.metric = arguments.reduce(binplus, spec[op_name]);
            } else {
        ...
        ArrayExpression: { // TODO: fix issue with arrays defined in anonymous functions being added to primary function stack
          "exit": function (p) {
            ...
            p.node.metric = arr.reduce(binplus, zero);
        ...
        BlockStatement: {
          "exit": function (p) {
            p.node.metric = p.node.body.map(e => e.metric).reduce(binplus, zero);
          }
        },
        VariableDeclaration: {
          "exit": function (p) {
            p.node.metric = p.node.declarations.map(e => e.metric).reduce(binplus, zero);
          }
        },
        VariableDeclarator: {
          "exit": function (p) {
        ...
        BinaryExpression: {
          "exit": function (p) {
            ...
            if (op in spec) {
              p.node.metric = [p.node.left, p.node.right].map(e => e.metric).reduce(binplus, spec[op]);
            } else {
        ...
        IfStatement: {
          "exit": function (p) {
            ...
            p.node.metric = plus(if_branch.metric, else_branch.metric);
          }
        },
      }
    });
