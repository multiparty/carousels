        ...
        CallExpression: {
          "exit": function (p) {
            ...
            if(op_name == "map" || op_name == "reduce"){
              ...
              var func = dict[arguments[1].name];
              var arr = arguments[0].elements;
              var arr_length = (op_name == "map") ? arr.length : arr.length - 1;

              p.node.metric = arr.reduce(plus, dot(polynomium.c(arr_length).toObject(), func));
