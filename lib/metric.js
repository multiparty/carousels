      ...
      visitor: {
        Program: {
          "exit": function (p) {
            ...
            // add body of program to children
            for (var i = 0; i < p.node.body.length; i++) {
              IR.children.push(p.node.body[i].IR);
            }
            p.node.metric = metric;
            p.node.results = results; // this is what the other file reads, put IR here when you want to use it
            p.node.IR = IR;
        ...
        AssignmentExpression: {
          "exit": function(p){
            ...
            console.log('assignment', p.node);
              p.node.left.IR.parent = "AssignmentExpr";
              p.node.left.IR.context = "Left";
              p.node.right.IR.parent = "AssignmentExpr";
        ...
        FunctionDeclaration: {
          "exit": function (p) {
            ...
            p.node.IR = {
              ...
              "context": "",
              "parent": "",
              "children": [p.node.body.IR]
            };
            console.log('function', p.node);;
          }
        },
        FunctionExpression: {
          "exit": function (p) {
        ...
        ArrayExpression: { // TODO: fix issue with arrays defined in anonymous functions being added to primary function stack
          "exit": function (p) {
            ...
            for (var i = 0; i < p.node.elements.length; i++) {
              ...
              p.node.IR.children.push(p.node.elements[i].IR);
            }
          }
        },
        Identifier: {
