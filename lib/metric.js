...
var createMetric = function(spec) {
  ...
  return function () {
    var zero = polynomium.c(0).toObject(), //create constant polynomium = 0
        ...
        dot = function (prod, node) { return polynomium.mul(prod, node.metric).toObject(); }
        ;
      ...
      visitor: {
          ...
          "exit": function (p) {
            ...
            if (op_name == "map" || op_name == "reduce") {
              ...
              var length_poly = (
                                ...
                                ? polynomium.v('b')
                                : polynomium.c(arr_length)
                                ;
              p.node.metric = polynomium.add(dot(length_poly.toObject(), func), arg.metric).toObject();
        ...
        ForStatement: {
          "exit": function (p) {
            ...
            // Compute depth
            let depth;  // polynomium object - usually a constant
            ...
            if (type === 'counterloop') {
              ...
              const cst_bound = polynomium.c(Math.floor((end-start)/inc));
              const var_bound = polynomium.v('b');
              depth = cst_bound.toObject();
            }
            ...
            // Total cost
            let weight = body.metric;
            weight = plus(weight, init);
            weight = plus(weight, test);
            weight = plus(weight, update);

            p.node.metric = polynomium.mul(depth, weight);
          }
        },
        IfStatement: {
          "exit": function (p) {
            p.node.metric = plus(p.node.consequent.metric, p.node.alternate);
          }
        },
      }
    });
