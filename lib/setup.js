function mode(str, spec, outDiv){ // modes for normal and preprocessing costs
  ...
  console.log("full IR", bbl.ast.program.IR);
  ...
  return parse_values(bbl_result);
}
...
function demo(specText, sel_param, preprocessing, inpText, outDiv) {
  ...
  var preproc = document.getElementById(preprocessing).checked;
      ...
      try {
        ...
        if(num_terms >1){
          if(param == "nothing"){
            ...
            plotslider(cost_analyzed, names, results.map((row,i)=> transpose(results[i])), "# of bits: ", "# of parties");
          }else{
