...
var createMetric = function (spec) {
  ...
  return function () {
    var zero = math.parse('0'),  // create constant polynomial always = 0
      ...
      max = function () {
        return new math.FunctionNode('max', Array.from(arguments));
      },
      binsum = function (a, b) {
        return sum(a, b);
      };
    ...
    return carousels.babelVisitorDefaults({
      visitor: {
        Program: {
          exit: function (p) {
            ...
            for (var i = 0; i < p.node.body.length; i++) {
              metric[p.node.body[i].id.name] = math.simplify(p.node.body[i].metric);
        ...
        CallExpression: {
          exit: function (p) {
            var op_name =  (p.node.callee.property != null) ? p.node.callee.property.name : p.node.callee.name;
            ...
            if (op_name === 'map' || op_name === 'reduce') {
              ...
              args = args.map(e => e.metric);
              p.node.metric = args.reduce(binsum, spec[op_name]);
            } else {
        ...
        MemberExpression:{
          exit: function (p) {
            if (p.container.arguments != null) {
              var caller_object = p.node.object;
              var args = p.container.arguments;
              args.unshift(caller_object);
